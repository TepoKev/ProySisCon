...
function enviar_partidas(e) {
    ...
    for (i = 0; i < partidas.length; i++) {
        ...
        for (c = 0; c < sel1.length; c++) {
            val1 = $(sel1[c]).val();
            val2 = $(sel2[c]).val();
            //ambos campos están vacios, o ambos campos están llenos
            if ((val1.length === 0 && val2.length === 0) || (val1.length > 0 && val2.length > 0))
            {
                e.preventDefault();
                alert("Solo puede llenar 1 cargo o abono por cada cuenta, saliendo");
                return;
            }
            // la cuenta actual será cargada
            if (val1.length > 0) {
                monto = val1;
                ops.push('c');
            } else { // la cuenta actual será abonada
                monto = val2;
                ops.push('a');
            }
...
function partida_modificada(partida) {
    ...
    if (cargos.length === abonos.length) {
        for (i = 0; i < cargos.length; i++) {
            ...
            c = $(cargos[i]).val();
            a = $(abonos[i]).val();
            //ambos están llenos, o ambos están vacios
            ...
            if (isNaN(c) || isNaN(a)) {
                ...
                partida_currupta = true;
                break;
            } else if ((a <= 0 && $(cargos[i]).val().length === 0) || (c <= 0 && $(abonos[i]).val().length === 0 )) {//no se aceptan valores negativos
                $(cargos[i]).addClass('is-invalid');
                $(abonos[i]).addClass('is-invalid');
                partida_currupta = true;
                //break;
